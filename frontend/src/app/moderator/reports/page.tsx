...
import { useModeratorReports, useHideForumTarget, useResolveReport } from "../_api";
...
export default function ModeratorReportsPage() {
  ...
  return (
    <div>
      <h1 className="display-face text-4xl font-black">Moderation Reports</h1>
      <div className="mt-5 grid gap-3">
        ...
        {reports.data?.map((report) => (
          <article className="border border-white/15 p-4 bg-white/5" key={report.id}>
            ...
            <p className="mt-2 text-sm italic">Reason: {report.reason}</p>
            <p className="mt-2 text-xs uppercase opacity-70">Reported by {report.reporter}</p>
          </article>
        ))}
      </div>
    </div>
