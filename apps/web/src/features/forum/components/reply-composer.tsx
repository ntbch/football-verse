...
export function ReplyComposer({
  ...
  return (
    <section className="mt-4 flex flex-col gap-4 border-t border-[var(--color-border)] pt-6">
      ...
      {locked ? (
        <div className="flex items-center justify-center gap-2 rounded-2xl border border-[var(--color-danger)] bg-[var(--color-danger)]/10 p-4 text-center">
          <p className="m-0 text-sm font-bold text-[var(--color-danger)]">
            This discussion thread has been locked. New replies are disabled.
          </p>
        </div>
      ) : authenticated ? (
        <form onSubmit={onSubmit} className="w-full">
          <div className="flex flex-col gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-background-surface)] p-5 shadow-sm">
            <textarea
              value={value}
        ...
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-background-surface)] p-6 py-6 text-center shadow-sm">
