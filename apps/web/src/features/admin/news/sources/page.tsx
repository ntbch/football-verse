...
export default function RssCrawlerPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (p: { name: string; feedUrl: string; sourceType: "RSS"; provider: "rss" }) => data<RssSource>(http.post("/admin/news/sources", p)),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: qk.admin.newsSources() });
      toast({ body: "Source removed. Sources with retained evidence are disabled instead of deleted.", type: "info" });
    },
  ...
  const crawlMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast({
        body: res.accepted
          ? "Synchronization accepted. New items will appear after background processing."
          : "Synchronization could not start. Check the ingestion service.",
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select value="RSS" disabled className="input !text-xs cursor-pointer">
              ...
              <div className="text-[10px] italic" style={{ color: "var(--color-text-secondary)" }}>
                RSS metadata only. Football Verse stores title, description, media, publisher and original URL without downloading the article body.
              </div>
