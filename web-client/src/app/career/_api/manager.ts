...
export const useAcceptManagerJob = (saveId: string) => {
  ...
  return useMutation({ mutationFn: (clubId: string) => http.post(game(`/saves/${saveId}/jobs/${clubId}/accept`)), onSuccess: () => { client.invalidateQueries({ queryKey: qk.game.save(saveId) }); client.invalidateQueries({ queryKey: qk.game.key("manager", saveId) }); client.invalidateQueries({ queryKey: qk.game.key("manager-jobs", saveId) }); } });
};
