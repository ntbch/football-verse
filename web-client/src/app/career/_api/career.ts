...
const saveMutation = (saveId: string, mutationFn: () => Promise<{ data: CareerSave }>) => {
  const client = useQueryClient();
  return useMutation({ mutationFn: () => body(mutationFn()), onSuccess: () => { client.invalidateQueries({ queryKey: qk.game.save(saveId) }); client.invalidateQueries({ queryKey: qk.game.key("squad", saveId) }); client.invalidateQueries({ queryKey: qk.game.standings(saveId) }); } });
};
export const useAdvanceDay = (saveId: string) => saveMutation(saveId, () => http.post(game(`/saves/${saveId}/advance-day`)));
export const useNextSeason = (saveId: string) => saveMutation(saveId, () => http.post(game(`/saves/${saveId}/next-season`)));
